...
自己可以创建 串行队列, 也可以创建 并行队列。看下面的代码\(代码已更新\)，它有两个参数，第一个上面已经说了，第二个才是最重要的。第二个参数用来表示创建的队列是串行的还是并行的，传入 DISPATCH\_QUEUE\_SERIAL 或 NULL 表示创建串行队列。传入 DISPATCH\_QUEUE\_CONCURRENT 表示创建并行队列。
